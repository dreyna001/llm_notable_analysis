...
- A Lambda image published to customer commercial AWS ECR in `us-east-1` and its immutable digest (`EcrRepositoryUri` + `ImageDigest`)
...
| **B — Customer-default** | On-prem `core,rag,analyst_portal` parity on commercial AWS |
...
7. [`docs/operations/deployment/COMMERCIAL_AWS_CUSTOMER_DEFAULT_DEPLOYMENT.md`](docs/operations/deployment/COMMERCIAL_AWS_CUSTOMER_DEFAULT_DEPLOYMENT.md) and [`deploy/aws/presets/`](deploy/aws/presets/) — copy preset, fill env, and `sam deploy`
...
Customer values checklist: [`docs/operations/deployment/COMMERCIAL_AWS_CUSTOMER_CONFIGURATION.md`](docs/operations/deployment/COMMERCIAL_AWS_CUSTOMER_CONFIGURATION.md) — required account, network, model, and secret inputs.
