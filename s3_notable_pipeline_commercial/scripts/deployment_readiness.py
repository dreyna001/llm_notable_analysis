#!/usr/bin/env python3
"""Validate commercial AWS customer-default inputs and write a deployment report."""
...
PARTITION = "aws"
REGION = "us-east-1"
PRODUCT = "llm-notable-analysis-commercial-aws"

REQUIRED_KEYS = (
    "AWS_ACCOUNT_ID", "COMMERCIAL_AWS_ACCOUNT_ID", "ECR_REPOSITORY_URI", "IMAGE_DIGEST",
    "BEDROCK_ANALYSIS_MODEL_ID", "BEDROCK_ANALYSIS_MODEL_ARN", "INPUT_BUCKET_NAME",
    "OUTPUT_BUCKET_NAME", "CASE_INDEX_TABLE_NAME", "PORTAL_UI_BUCKET_NAME",
    "PORTAL_JWT_ISSUER", "PORTAL_JWT_AUDIENCE", "PORTAL_CORS_ALLOWED_ORIGINS",
    "OPENSEARCH_ENDPOINT", "OPENSEARCH_DOMAIN_ARN", "RAG_TENANT_ID",
    "CUSTOMER_VPC_SUBNET_IDS", "CUSTOMER_SECURITY_GROUP_IDS",
)
...
def validate_values(values: dict[str, str]) -> list[str]:
    messages = [f"missing required value: {key}" for key in REQUIRED_KEYS if not values.get(key, "").strip()]
    ...
    if values.get("COMMERCIAL_AWS_ACCOUNT_ID") != account_id:
        messages.append("COMMERCIAL_AWS_ACCOUNT_ID must match AWS_ACCOUNT_ID")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "customer_actions_after_deploy": [
            ...
            "Ingest approved knowledge corpora", "Upload the analyst portal build",
