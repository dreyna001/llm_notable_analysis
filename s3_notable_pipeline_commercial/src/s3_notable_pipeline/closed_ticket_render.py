...
class ClosedTicketChunkRecord:
    ...
    def search_text(self) -> str:
        """Return lexical search text stored next to the embedding."""
...
def build_closed_ticket_chunk_id(*, ticket_id: str, section: str, ordinal: int) -> str:
        ...
        if set(keys) <= {"display_value", "value", "link"} and (
            ...
            for key in sorted(k for k in keys if k in {"display_value", "value"}):
                ...
                if isinstance(leaf_value, (dict, list)):
                    yield from _iter_leaf_values(leaf_value, leaf_path, depth=depth + 1)
                else:
...
def build_closed_ticket_chunks(
    ...
    *,
    ...
    budget = _config_int(
        config, "CLOSED_TICKET_MAX_INDEX_CHUNKS_PER_TICKET", _MAX_INDEX_CHUNKS_DEFAULT
    )
    ordinal = 0
