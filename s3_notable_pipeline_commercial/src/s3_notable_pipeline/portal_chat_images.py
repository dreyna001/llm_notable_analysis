...
_BEDROCK_FORMAT_BY_MEDIA_TYPE: Final[dict[str, str]] = {
    "image/png": "png",
    "image/jpeg": "jpeg",
    "image/webp": "webp",
    "image/gif": "gif",
...
def validated_image_bytes(image: ValidatedChatImage) -> bytes:
    """Return sanitized image bytes from one validated chat image."""
    prefix = f"data:{image.media_type};base64,"
    if not image.data_url.startswith(prefix):
        raise ValueError("Invalid or corrupted image.")
    encoded = image.data_url[len(prefix) :]
    try:
...
def build_bedrock_converse_user_content(
    prompt: str,
    images: tuple[ValidatedChatImage, ...],
) -> list[dict[str, Any]]:
    """Build Bedrock Converse user content blocks for one multimodal call."""
    content: list[dict[str, Any]] = [{"text": prompt}]
    for image in images:
        bedrock_format = _BEDROCK_FORMAT_BY_MEDIA_TYPE.get(image.media_type)
        ...
        content.append(
            {
                "image": {
                    ...
                    "source": {"bytes": validated_image_bytes(image)},
                }
            }
        )
    return content
