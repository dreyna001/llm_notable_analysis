...
from azure_notable_pipeline.portal_chat_images import (
...
def _portal_test_config(**overrides: object) -> Config:
    values = {
        "PORTAL_ENABLED": True,
...
def _chat_image_config(**enabled: bool) -> Config:
    return _portal_test_config(
        CASE_QA_CHAT_IMAGES_ENABLED=enabled.get("enabled", True),
    )


class TestPortalChatImages(unittest.TestCase):
    def test_defaults_disable_chat_images(self) -> None:
        config = _portal_test_config(CASE_QA_CHAT_IMAGES_ENABLED=False)
    ...
    def test_validate_rejects_oversized_decoded_bytes(self) -> None:
        config = _portal_test_config(
            CASE_QA_CHAT_IMAGES_ENABLED=True,
            CASE_QA_MAX_CHAT_IMAGE_BYTES=10,
        )
        with self.assertRaisesRegex(ValueError, "10 B or smaller"):
            validate_chat_images([_png_payload()], config)

    def test_validate_rejects_dimension_and_pixel_limits(self) -> None:
        config = _portal_test_config(
            CASE_QA_CHAT_IMAGES_ENABLED=True,
            CASE_QA_MAX_CHAT_IMAGE_DIMENSION=16,
            CASE_QA_MAX_CHAT_IMAGE_PIXELS=256,
        )
