...
from .azure_search_retrieval import config_value
...
def _extract_pdf_text(data: bytes, config: Any) -> str:
    ...
    if text is None:
        raise ValueError(
            "PDF extraction requires pypdf or pdfminer.six; install one dependency in the Function image"
        )
    return text
...
def _extract_image_text(data: bytes, suffix: str, config: Any) -> str:
    use_document_intelligence = _bool_config(
        config, "IMAGE_INGEST_USE_DOCUMENT_INTELLIGENCE", False
    )
    if use_document_intelligence:
        return _extract_image_with_document_intelligence(data, config)

    return _extract_image_placeholder(data, suffix, config)


def _extract_image_with_document_intelligence(data: bytes, config: Any) -> str:
    endpoint = str(config_value(config, "AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT", "") or "").strip().rstrip("/")
    if not endpoint:
        raise ValueError(
    ...
    api_key = str(config_value(config, "AZURE_DOCUMENT_INTELLIGENCE_API_KEY", "") or "").strip()
    if api_key:
    ...
    lines = [
        str(line.content or "").strip()
        for page in getattr(result, "pages", []) or []
        for line in getattr(page, "lines", []) or []
        if str(getattr(line, "content", "") or "").strip()
    ]
    if not lines:
        raise ValueError("Document Intelligence returned no text for image document")
    return "\n".join(lines)
...
def _extract_image_placeholder(data: bytes, suffix: str, config: Any) -> str:
    ...
    return (
        ...
        "enable IMAGE_INGEST_USE_DOCUMENT_INTELLIGENCE for text extraction]"
    )
