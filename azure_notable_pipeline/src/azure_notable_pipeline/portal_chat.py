"""Azure OpenAI answer synthesis for portal Case Q&A."""
...
from .azure_openai_gateway import (
...
_ANSWER_ACTION_CLAIM_RE = re.compile(
    r"\b(i|we|the portal|this portal|the assistant|the system)\s+"
    r"(?:have\s+|has\s+|just\s+|successfully\s+)?"
    ...
    r"remediated|restarted|wrote|posted|submitted)\b|"
    r"\b(ticket|incident|notable|splunk search|query|playbook|firewall rule|"
    r"edr action|endpoint action)\s+(?:has\s+been|was)\s+"
    ...
    r"restarted|run|executed|written|posted|submitted)\b",
...
_GENERAL_OUT_OF_SCOPE_RE = re.compile(
    r"^\s*(?:out of scope:|this question is outside .*?(?:technology|technical)|"
...
_ANSWER_CITATION_RE = re.compile(
    r"(?:\(?\s*sources?\s*(?:[#:]|no\.?|number)?\s*\d+"
    r"(?:\s*[-–,]\s*\d+)*\s*\)?|"
    r"\[\s*sources?\s*(?:[#:]|no\.?|number)?\s*\d+"
    r"(?:\s*[-–,]\s*\d+)*\s*\]|"
    r"\(\s*#\s*\d+(?:\s*[-–,]\s*\d+)*\s*\)|"
    r"\[\s*#\s*\d+(?:\s*[-–,]\s*\d+)*\s*\]|"
    r"\bsources?\s*#\s*\d+\b|\bsources?\s+\d+\b|"
...
def _complete_markdown_response(
    *,
    prompt: str,
    config: Config,
    ...
    response = create_chat_completion(
        ...
        temperature=0.0,
        timeout_seconds=config.PORTAL_CHAT_TIMEOUT_SEC,
    )
    if response.get("tool_calls"):
        raise AzureOpenAIResponseError(
            "Azure OpenAI portal chat attempted an unconfigured tool call"
        )
...
def complete_markdown_answer(
    *,
    ...
    return _complete_markdown_response(
        prompt=prompt,
        config=config,
...
def finalize_general_knowledge_answer(
    *,
    ...
    if _GENERAL_OUT_OF_SCOPE_RE.search(answer):
        return PortalAnswer(answer=answer, answer_status="unknown", context_usage=context_usage)
    return PortalAnswer(answer=answer, answer_status="answered", context_usage=context_usage)
...
def _case_grounded_system_prompt_chars() -> int:
    prompt = build_case_grounded_prompt(question="", sources=[], conversation_history=None)
...
__all__ = [
    ...
    "finalize_general_knowledge_answer",
