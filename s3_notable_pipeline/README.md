...
### AWS analysis output contract (current default)

`s3_notable_pipeline` is the primary AWS analyzer path and currently uses the same generated-field contract as `s3_testing`:
...
### Candidate future sections (not generated by default)

These sections are intentionally documented here for future expansion planning, but are not required in the current AWS contract.

The details below are reference-only and capture the prior expanded contract and prompt guidance so we can re-enable safely later.

#### `attack_chain` (future)

Sub-fields from prior contract:
- `likely_previous_steps`: list of exactly 1 object with:
  - `tactic_id`, `tactic_name`, `mitre_url`, `why_this_step`, `what_to_check`, `uncertainty_alternatives`
  - `investigation_tree`: object with exactly `Q1`, `Q2`, `Q3` (each with `question`)
- `likely_next_steps`: same shape as `likely_previous_steps`
- `kill_chain_phase`: string
- `tactic_span_note`: string

Legacy prompt text used for this section:

```text
ATTACK CHAIN:
- ALWAYS include EXACTLY 1 previous step and EXACTLY 1 next step as hypotheses.
- TIME WINDOWS: If ALERT_TIME is provided, previous queries use earliest=-24h@h latest=ALERT_TIME, next queries use earliest=ALERT_TIME latest=+24h@h.
```

#### `containment_playbook` (future)
...
When proposing mitigations, do NOT list generic best practices.
...
4) Cite ATT&CK mitigations by ID where relevant (e.g., M1032, M1026, M1030)
```

#### `splunk_enrichment` (future)

Sub-fields from prior contract:
- list of 1-4 objects, each with:
  - `phase`: `"previous"` or `"next"`
  - `supports_tactic`: string
  - `purpose`: string
  - `query`: string (SPL; index/sourcetype as `PLACEHOLDER`)
...
- `disambiguation_checklist`: list of 3-5 strings
...
- Provide a "Primary Tactic (most plausible)" and "Secondary Tactic(s) (possible)".

Rules:
- Do not label a tactic as "Primary" unless the notable contains direct supporting evidence.
- If multiple tactics fit, keep "tactic span" and state what additional fields/logs would disambiguate.
- If required context is missing, state "unknown".
```

#### `benign_explanations` (future)

Sub-fields from prior contract:
- list of 3-5 objects, each with:
  - `hypothesis`: string
  - `expect_if_true`: list of 1-2 strings
  - `argue_against`: string
  - `best_validation`: string
...
For each hypothesis include: hypothesis, expect_if_true, argue_against, best_validation.
Keep it concise and do not assume extra telemetry unless you name it explicitly.
```

#### `context_enrichment` (future)

Sub-fields from prior contract:
- `enrichment_steps`: list of objects with `field`, `enrichment_type`, `result_or_status`
- `baseline_queries`: list of objects with `purpose`, `query`
- `limitations`: string
...
When a notable includes network-origin fields:
- Enrichment: geo, ASN/org ownership, internal_vs_external, known_egress, reputation (or "not available")
- Baseline: propose queries for normal actor->target patterns and unusual timing/volume
- Output: state enriched context (or not available), use baseline to set confidence, list limitations
```
