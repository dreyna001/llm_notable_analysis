...
from s3_notable_pipeline.kb_document_extract import extract_document_text, image_ingest_enabled
from s3_notable_pipeline.rag_ingestion import parse_s3_document
...
def media_config(**overrides):
    values = {
        ...
        "IMAGE_INGEST_USE_TEXTRACT": False,
    }
...
class KbDocumentExtractTests(unittest.TestCase):
    ...
    def test_extract_png_placeholder_without_textract(self):
    ...
    @patch("s3_notable_pipeline.aws_clients.aws_client")
    def test_extract_image_with_textract(self, aws_client_mock):
        textract_client = SimpleNamespace(
            detect_document_text=lambda **kwargs: {
                "Blocks": [
                    {"BlockType": "LINE", "Text": "Escalation diagram"},
                ]
            }
        )
        aws_client_mock.return_value = textract_client
        text = extract_document_text(
            MINIMAL_PNG,
            "png",
            media_config(IMAGE_INGEST_USE_TEXTRACT=True),
        )
        self.assertEqual(text, "Escalation diagram")
        aws_client_mock.assert_called_once_with("textract")


class ParseS3DocumentMediaTests(unittest.TestCase):
    def test_parse_png_when_image_ingest_enabled(self):
        s3_client = FakeS3Client({"rag-sources/soc/diagram.png": MINIMAL_PNG})
        text = parse_s3_document(
            bucket="docs",
            key="rag-sources/soc/diagram.png",
            s3_client=s3_client,
            version_id="v1",
            config=media_config(),
            max_bytes=1_048_576,
    ...
    def test_parse_png_rejected_when_image_ingest_disabled(self):
        s3_client = FakeS3Client({"rag-sources/soc/diagram.png": MINIMAL_PNG})
