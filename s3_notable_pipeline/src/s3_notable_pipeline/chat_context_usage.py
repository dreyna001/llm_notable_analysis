...
def _conversation_chars(conversation_history: Sequence[Any] | None) -> int:
    if not conversation_history:
        return 0
    from .portal_chat import _render_conversation_history
...
def _lane_block_chars(sources: Sequence[Any] | None, lane: str) -> int:
    if not sources:
        return 0
    from .portal_chat import _format_context_block

    total = 0
    for source in sources:
        if not isinstance(source, dict):
            continue
        if str(source.get("source_lane") or "") != lane:
            continue
...
def build_context_usage(
    ...
    *,
    ...
    if prompt_text is not None:
        ...
        prompt_tokens, estimate_method = estimate_tokens_from_text(prompt_text)
    else:
