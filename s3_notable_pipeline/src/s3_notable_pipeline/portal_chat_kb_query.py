...
def _case_chunk_text(chunk: dict[str, Any]) -> str:
    return str(chunk.get("search_text") or chunk.get("text") or "").strip()


def build_case_aware_kb_query(
    question: str,
    *,
    case_chunks: Sequence[dict[str, Any]] | None = None,
    ...
    for chunk in case_chunks or ():
        lane = str(chunk.get("source_lane") or "current_case").strip()
        if lane != "current_case":
            continue
        text = _case_chunk_text(chunk)
        if text:
