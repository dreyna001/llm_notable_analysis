"""Configuration loading for Dockerized on-prem notable analysis service.

Loads configuration from environment variables (typically via config.env).
All paths default to container-mounted locations.
"""
...
class Config:
    ...
    INCOMING_DIR: Path = field(default_factory=lambda: Path("/watch/incoming"))
    PROCESSED_DIR: Path = field(default_factory=lambda: Path("/watch/processed"))
    QUARANTINE_DIR: Path = field(default_factory=lambda: Path("/watch/quarantine"))
    REPORT_DIR: Path = field(default_factory=lambda: Path("/watch/reports"))
    ARCHIVE_DIR: Path = field(default_factory=lambda: Path("/watch/archive"))

    # Polling interval (seconds) for file_drop mode
    POLL_INTERVAL: int = 5

    # Local LLM (OpenAI-compatible inference container)
    LLM_API_URL: str = "http://model-serving:8000/v1/chat/completions"
    LLM_API_TOKEN: str = ""
    LLM_MODEL_NAME: str = "gpt-oss-120b"
    LLM_MAX_TOKENS: int = 4096
    LLM_TIMEOUT: int = 420  # seconds

    # Optional retrieval grounding (RAG)
    RAG_ENABLED: bool = False
    RAG_SQLITE_PATH: Path = field(
        default_factory=lambda: Path("/kb/index/kb.sqlite3")
    )
    RAG_FAISS_PATH: Path = field(
        default_factory=lambda: Path("/kb/index/kb.faiss")
    )
    ...
    # Conservative default profile for gpt-oss-120b:
    # Start sequential/small queue and increase only after validating host headroom.
...
def load_config() -> Config:
    ...
    return Config(
        ...
        INCOMING_DIR=Path(os.getenv("INCOMING_DIR", "/watch/incoming")),
        PROCESSED_DIR=Path(os.getenv("PROCESSED_DIR", "/watch/processed")),
        QUARANTINE_DIR=Path(os.getenv("QUARANTINE_DIR", "/watch/quarantine")),
        REPORT_DIR=Path(os.getenv("REPORT_DIR", "/watch/reports")),
        ARCHIVE_DIR=Path(os.getenv("ARCHIVE_DIR", "/watch/archive")),
        POLL_INTERVAL=int(os.getenv("POLL_INTERVAL", "5")),
        LLM_API_URL=os.getenv(
            "LLM_API_URL", "http://model-serving:8000/v1/chat/completions"
        ),
        LLM_API_TOKEN=os.getenv("LLM_API_TOKEN", ""),
        LLM_MODEL_NAME=os.getenv("LLM_MODEL_NAME", "gpt-oss-120b"),
