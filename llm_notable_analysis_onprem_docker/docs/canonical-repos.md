...
This document points to the **Git repository** (compose, docs, scripts) and the **GHCR images** used for this deployment. Maintainer namespace: **`dreyna001`**. If you use a fork or different GitHub user, replace that name in URLs and image names.
...
| **This Docker bundle in the repo** | […/tree/main/llm_notable_analysis_onprem_docker](https://github.com/dreyna001/llm_notable_analysis/tree/main/llm_notable_analysis_onprem_docker) |
...
cd llm_notable_analysis/llm_notable_analysis_onprem_docker
```
...
|-------|-------------------|--------|
| **Analyzer** (this app) | `ghcr.io/dreyna001/notable-analyzer:1.0.0` | Built from `Dockerfile.analyzer` in this folder; tag may vary (check Packages). |
| **llama.cpp server** (mirrored) | `ghcr.io/dreyna001/llama-cpp-server:server` | Retag/push of upstream `ghcr.io/ggml-org/llama.cpp:server`; create this package when you mirror. |
