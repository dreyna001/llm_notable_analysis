...
class TTPValidator:
    ...
    def _load_valid_ttps(self, ids_file_path: Path):
        ...
        try:
            ...
            total_ttps = len(self._valid_subtechniques) + len(
                self._valid_parent_techniques
            )
            logger.info(
                f"Loaded {len(self._valid_subtechniques)} valid sub-techniques and {len(self._valid_parent_techniques)} parent techniques (total: {total_ttps})"
            )
    ...
    def is_valid_ttp(self, ttp_id: str) -> bool:
        ...
        """
        return (
            ttp_id in self._valid_subtechniques
            or ttp_id in self._valid_parent_techniques
        )
    ...
    def get_valid_ttps_for_prompt(self) -> str:
        ...
        """
        all_ttps = sorted(list(self._valid_subtechniques)) + sorted(
            list(self._valid_parent_techniques)
        )
        return ", ".join(all_ttps)

    def filter_valid_ttps(
        self, scored_ttps: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
