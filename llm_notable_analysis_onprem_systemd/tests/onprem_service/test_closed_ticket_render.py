...
class TestClosedTicketRender(unittest.TestCase):
    ...
    def test_chunk_ids_are_stable(self) -> None:
        ...
        self.assertEqual(chunk_id, build_closed_ticket_chunk_id(
            ticket_id="ticket-a",
            section="ticket.core",
            ordinal=0,
        ))
    ...
    def test_attachment_from_other_ticket_is_ignored(self) -> None:
        record = _record(ticket_id="ticket-a")
        attachment = ClosedTicketAttachmentRecord(
            attachment_id="att-2",
            ticket_id="ticket-b",
            filename="other.txt",
            content_type="text/plain",
            metadata={},
            semantic_text="Should not appear",
            extraction_status="text_decoded",
        )
        chunks = build_closed_ticket_chunks(
            record,
        ...
        self.assertFalse(any(chunk.section == "attachment.semantic" for chunk in chunks))
