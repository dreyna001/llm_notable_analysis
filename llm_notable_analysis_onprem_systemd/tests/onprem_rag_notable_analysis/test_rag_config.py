...
class TestRAGConfig(unittest.TestCase):
    def test_default_embedding_model_is_bge(self) -> None:
        """RAG defaults should match the on-prem BGE embedding target."""
        config = RAGConfig()

        self.assertEqual(config.backend, "postgres")
        self.assertEqual(config.embedding_model_name, "BAAI/bge-base-en-v1.5")
