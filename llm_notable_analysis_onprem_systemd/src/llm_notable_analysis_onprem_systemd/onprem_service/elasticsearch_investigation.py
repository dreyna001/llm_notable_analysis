...
This module validates generated Elasticsearch Query DSL requests against
deterministic policy rules and executes eligible requests through `_search`.
"""
...
def validate_elasticsearch_query_policy(
    ...
    *,
    ...
    if not index_pattern:
        return False, "query must include index_pattern"
    ...
    configured_timeout = int(
        getattr(config, "ELASTICSEARCH_TIMEOUT_SECONDS", _DEFAULT_TIMEOUT_SECONDS)
    )
...
def execute_elasticsearch_query(
    ...
    *,
    ...
    if not config.ELASTICSEARCH_BASE_URL or not config.ELASTICSEARCH_API_KEY:
        return {
            "status": "error",
            "executor": "elasticsearch",
            "query": query_text,
    ...
    verify_tls = config.ELASTICSEARCH_CA_BUNDLE if config.ELASTICSEARCH_CA_BUNDLE else True
    ...
    try:
        ...
        try:
            ...
            return {
                ...
                "executor": "elasticsearch",
                "query": query_text,
                "message": "Elasticsearch response was not valid JSON",
            }
