...
Production-oriented retrieval uses PostgreSQL FTS + pgvector. Each KB lane has
its own source directory, ingest artifact directory, and Postgres chunks table.
Do not mix Splunk or Elastic query tokens into `source_docs`; keep them in the
dedicated lanes below.

| Lane | Source docs | Ingest artifacts | Default table (`config.env.example`) |
|------|-------------|------------------|--------------------------------------|
| General SOC | `.../knowledge_base/source_docs` | `.../knowledge_base/index` | `RAG_POSTGRES_CHUNKS_TABLE` (`kb_chunks`) |
| SPL query | `.../knowledge_base/spl_query_source_docs` | `.../knowledge_base/spl_query_index` | `SPL_QUERY_RAG_POSTGRES_CHUNKS_TABLE` (`spl_query_chunks`) |
| Elasticsearch query | `.../knowledge_base/elasticsearch_source_docs` | `.../knowledge_base/elasticsearch_index` (operator-chosen; not a config key) | `ELASTICSEARCH_GROUNDING_POSTGRES_CHUNKS_TABLE` (`elasticsearch_query_chunks`) |

All paths above default under `/opt/llm-notable-analysis/knowledge_base/`.
Override SPL paths with `SPL_QUERY_RAG_SOURCE_DIR` and `SPL_QUERY_RAG_INDEX_DIR`.
Override Elastic source docs with `ELASTICSEARCH_GROUNDING_SOURCE_DIR` (runtime
retrieval only; ingest still needs matching `--source-dir`).

**Ingest entry points**

- General SOC and SPL query: `scripts/setup_postgres_rag.sh` (run from the install
  root, typically `/opt/llm-notable-analysis/`). SPL mode: add `--spl-query-rag`;
  paths and table come from `config.env` unless overridden with `--source-dir` /
  `--index-dir`.
- Elasticsearch query: `onprem_rag_notable_analysis.future.corpus_ingest` directly
...
default to `RAG_BACKEND=postgres`.
...
2. Use clear filenames and headings. Good examples:
   ...
   escalation_contacts_runbook.txt
   ```

   Do not put Splunk index/sourcetype catalogs here; use `spl_query_source_docs`.

3. Rebuild the general SOC Postgres table (from `/opt/llm-notable-analysis/`):
   ...
   After the database and schema exist, content-only rebuilds may use
   `--skip-db-setup`.
...
2. Rebuild the SPL query table (uses `SPL_QUERY_RAG_SOURCE_DIR`,
   `SPL_QUERY_RAG_INDEX_DIR`, and `SPL_QUERY_RAG_POSTGRES_CHUNKS_TABLE` from
   ...
   Use `--skip-db-setup` for ingest-only rebuilds after initial provisioning.
...
2. Rebuild the Elasticsearch query table with corpus ingest. Align
   `--source-dir` with `ELASTICSEARCH_GROUNDING_SOURCE_DIR` and pass
   `--postgres-chunks-table` matching `ELASTICSEARCH_GROUNDING_POSTGRES_CHUNKS_TABLE`
   (defaults to `elasticsearch_query_chunks`; `corpus_ingest` does not read the
   Elastic table name from config automatically):
   ...
   There is no `ELASTICSEARCH_GROUNDING_INDEX_DIR` config key; choose a stable
   `--index-dir` and keep it consistent across rebuilds. Run
   `scripts/setup_postgres_rag.sh` once without `--skip-db-setup` if pgvector or
   the shared Postgres schema are not provisioned yet.
...
- Remove stale or superseded docs before rebuilding, or move them to an
  operator-controlled archive outside the active source directory for that lane.
...
- `RAG_EMBEDDING_MODEL` or `RAG_VECTOR_DIMENSIONS` change (rebuild **all three**
  lanes; reranker-only changes do not require KB rebuild)
...
There is no scheduled KB rebuild unit in the current package. Rebuilds are manual:

- General SOC: `setup_postgres_rag.sh` (default paths)
- SPL query: `setup_postgres_rag.sh --spl-query-rag`
- Elasticsearch query: `corpus_ingest` with Elastic paths and table

## Rollback

Ingest writes `chunks.jsonl` and `ingest_report.json` under each lane's index
directory. Postgres rebuilds **delete all rows** in the target chunks table, then
re-insert from the current source set (same transaction per table).

To roll back bad KB content:

1. Remove or replace the bad source document in the affected lane directory
   (`source_docs`, `spl_query_source_docs`, or `elasticsearch_source_docs`).
2. Restore the prior approved document set from operator source control or backup.
3. Rerun the matching ingest command for that lane only.
4. Run `scripts/smoke_service_chain.sh` when services are up.

Keep authoritative source documents in a controlled location outside the runtime
host if rollback auditability is required.
