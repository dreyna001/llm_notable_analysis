...
1. **[End-to-end behavior](delivery_package/EXECUTIVE_ONPREM_WORKFLOW.md)** — File-drop flow, optional RAG (advisory context), SPL generation, read-only Splunk execution, Splunk writeback, ServiceNow draft/create and approval gates. Read this **first** for what the service does.

2. **[Operations guide index](operations/README.md)** — Customer-facing tuning guides organized by area: SPL, KB, RAG, LLM inference, file-drop/retention, Splunk writeback, ServiceNow, MITRE, security, install, prestage, and recovery.

3. **[Installation](operations/INSTALL.md)** — Bring-up with `scripts/install.sh`, prerequisites, manual follow-ups.

4. **[Runtime env contract](../config.env.example)** — All feature flags and integration settings live here with section comments (`RAG_*`, `SPL_QUERY_RAG_*`, Splunk investigation, ServiceNow `SERVICENOW_*`). Copy to `/etc/notable-analyzer/config.env` on the host.

5. **[Knowledge base / RAG operations](operations/KNOWLEDGE_BASE_OPERATIONS.md)** and **[RAG tuning](operations/RAG_OPERATIONS.md)** — Source document lifecycle plus retrieval configuration. Only needed when `RAG_ENABLED=true`.

6. **[Deployment readiness (executive gateway)](delivery_package/AIOPTIMIZED_SOC_ANALYSIS_ONPREM_READINESS_OVERVIEW.md)** and **[readiness checklist](delivery_package/AIOPTIMIZED_SOC_ANALYSIS_ONPREM_READINESS_ASSESSMENT.md)** — Ownership, approvals, Splunk/RAG prerequisites before production.
...
| Splunk search execution (read-only) | [EXECUTIVE_ONPREM_WORKFLOW.md § Splunk](delivery_package/EXECUTIVE_ONPREM_WORKFLOW.md), [SPL_OPERATIONS.md](operations/SPL_OPERATIONS.md), [`config.env.example`](../config.env.example) (`INVESTIGATION_*`, `SPLUNK_SEARCH_*`) |
