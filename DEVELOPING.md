...
- `s3_notable_pipeline` (shared direct-Bedrock analyzer used by preview file drop)
...
When `config.portal-preview.env` selects Bedrock, the preview API also watches
`llm_notable_analysis_onprem_systemd/wsl-notable-data/incoming/`. Drop a
top-level `.json` or `.txt` alert there; direct Bedrock analysis writes a report
under `wsl-notable-data/reports/`, moves the input to `processed/` or
`quarantine/`, and adds the completed analysis to the live case view.
