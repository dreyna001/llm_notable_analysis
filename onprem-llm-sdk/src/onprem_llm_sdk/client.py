...
from .contracts import CompletionRequest, CompletionResult, parse_completion_text, parse_retry_after_seconds
from .errors import (
    ClientRequestError,
    RateLimitError,
    RequestTimeoutError,
...
class VLLMClient:
    ...
    def _inflight_slot(self) -> Generator[None, None, None]:
        ...
        with self._inflight_lock:
            self._inflight += 1
            self._metrics.record_inflight(app_name=self._config.llm_app_name, inflight=self._inflight)
        try:
            yield
        finally:
            with self._inflight_lock:
    ...
    def complete(
        ...
        *,
        ...
        """Send a completion request with bounded retries and consistent error mapping."""
        ...
        with self._inflight_slot():
            for attempt in range(1, max_attempts + 1):
                ...
                if retryable and attempt < max_attempts:
                    self._retry_sleep(attempt, response.headers if response.status_code == 429 else None)
                    continue
